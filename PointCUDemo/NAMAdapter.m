...
- (void)loadAd
{
    if (_adType == SSPAdBannerType) {
        ...
        [self setupReactNativeAd:@"ReactNative"];
    }
...
-(void)isNotSupport:(SSPAdType) adType {
    switch (adType) {
        case SSPAdBannerType:
            NSLog(@"NAMAdapter is not support :%u", _adType);
...
-(void)setupNativeAd:(NSString*) typeName {
    if(_integrationKey != nil)
    {
        ...
        // timeOut
        if(namNativeAdRenderer.timeOut) {
            adLoader.requestTimeoutInterval = namNativeAdRenderer.timeOut;
        }
        
        // native 심플형
        GFPNativeSimpleAdRenderingSetting *simpleSetting = namNativeAdRenderer.GFPNativeSimpleSetting;

        if(simpleSetting == nil) {
            simpleSetting = [[GFPNativeSimpleAdRenderingSetting alloc] init];
            simpleSetting.preferredAdChoicesViewPosition = GFPAdChoicesViewPositionTopRightCorner;
        }
        simpleSetting.adChoicesPositionInFullAdView = YES;
        ...
        // Native 일반형
        GFPNativeAdRenderingSetting *setting = namNativeAdRenderer.GFPNativeSetting;
        
        if(setting == nil) {
            setting = [[GFPNativeAdRenderingSetting alloc] init];
            setting.preferredAdChoicesViewPosition = GFPAdChoicesViewPositionTopRightCorner;
        }
        if (namNativeAdRenderer.backgroundBlur) {
            setting.enableMediaBackgroundBlur = namNativeAdRenderer.backgroundBlur;
        }
        ...
        // Video 광고 타입일 경우
        GFPVideoOptions *videoOption = [[GFPVideoOptions alloc] initWithPlayPolicy:GFPVideoPlayPolicyAutoPlay viewType:GFPVideoViewTypeLanding useCustomControlView:YES];
        ...
        adLoader.delegate = self;
        if (namNativeAdRenderer.namDedupeManager != nil) {
            NSLog(@"NAMAdapter use GFPAdDedupeManager load");
            [namNativeAdRenderer.namDedupeManager loadAdWith:adLoader];
        } else {
            [adLoader loadAd];
        }
    }
...
-(void)setupReactNativeAd:(NSString*) typeName {
    if(_integrationKey != nil)
    {
        NSString *adUnitID = [_integrationKey valueForKey:@"NamUnitId"];
...
-(void)setupModal:(NSString*) typeName {
    if (_integrationKey != nil)
    {
        NSString *adUnitID = [_integrationKey valueForKey:@"NamUnitId"];
...
-(void)nativeAdWasMuted:(GFPNativeAd *)nativeAd {
    NSLog(@"NAMAdapter GFPNativeAdDelegate nativeAdWasMuted");
